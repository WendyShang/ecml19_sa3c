...
class A3C_LSTM_SA3C(Model):
    def __init__(self, args):
        super(A3C_LSTM_SA3C, self).__init__(args)
        self.sig   = args.sig
    ...
    def forward(self, x, lstm_hidden_vb=None, eps=None):
        ...
        y = F.leaky_relu(torch.cat([x,z], dim=1))
        p = self.policy_5(y)
        p = self.policy_6(p)
        v = self.value_5(y)
        return p, v, (hx, cx)
...
class A3C_LSTM_SA3C_CRELU(Model):
    def __init__(self, args):
        super(A3C_LSTM_SA3C_CRELU, self).__init__(args)
        self.sig   = args.sig
