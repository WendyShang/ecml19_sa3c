...
A3C_Experience = namedtuple('A3C_Experience', 'staten ,state0, action, reward, state1, terminal1, policy_vb, value0_vb, mu_vb, sigma_vb, posterior_vb, prior_vb')
...
def klLoss(post, prior):
    mu = post[0]
    logvar = post[1]
    mu_prior = prior[0]
    logvar_prior = prior[1]
    mean_diff_sq = (mu - mu_prior).pow(2)
    var = logvar.exp()
    var_prior = logvar_prior.exp() 
    KLD_element = -0.5 * (1+logvar) + 0.5 * logvar_prior + 0.5 * (var + mean_diff_sq) / var_prior
...
class A3CAgent(Agent):
    ...
    def _reset_lstm_hidden_vb_episode(self, training=True): 
        ...
        if not_training:
            with torch.no_grad():
                ...
                self.latent = Variable(torch.zeros(self.batch_size, self.hidden_dim))
    ...
    def fit_model(self):
        ...
        for episode in range(self.episode_start, self.episode_number):
            ...
            for step in range(self.rollout_steps):
                ...
                action, p_vb, v_vb, posterior, prior = self._forward(states)
                def step_game(game):
                    id = game.get_id()
                    if not experiences[id].terminal1:
                        ...
                        self.A3C_Experiences[id].posterior_vb.append((posterior[0][id].unsqueeze(0),posterior[1][id].unsqueeze(0)))
                        self.A3C_Experiences[id].prior_vb.append((prior[0][id].unsqueeze(0),prior[1][id].unsqueeze(0)))
    ...
    def _test_and_save(self, episode, test_game):
        ...
        total_reward = 0
        with torch.no_grad():
            test_lstm_hidden_vb = (Variable(torch.zeros(1, self.hidden_dim).type(self.dtype)),
                                   Variable(torch.zeros(1, self.hidden_dim).type(self.dtype)))
            test_latent_vb = Variable(torch.zeros(1, self.hidden_dim).type(self.dtype))
            ...
            while eps_count < self.max_episode and not test_done:
                test_p_vb, test_v_vb, test_latent_vb, test_lstm_hidden_vb, _, _ = self.model(
                    preprocess_state(current_experience.state1, self.dtype, is_volatile=False), test_latent_vb,
                    test_lstm_hidden_vb)
    ...
    def _forward(self, state_vb):
        p_vb, v_vb, self.latent, self.lstm_hidden_vb, (mu, sigma), (mu_prior, sigma_prior) = self.model(state_vb, self.latent, self.lstm_hidden_vb)
        if self.training:
        ...
        return action,torch.clamp(p_vb, min=0.000001, max=0.999999),v_vb, (mu, sigma), (mu_prior, sigma_prior)

    def _backward(self, sT_vb):
        self.optimizer.zero_grad()
        # preparation
        _, valueT_vb, _, _, _, _ = self.model(sT_vb, self.latent, self.lstm_hidden_vb)
        for i in range(self.batch_size):
        ...
        kl_loss_vb = [0. for i in range(self.batch_size)]
        loss_model_vb = 0
        for j in range(self.batch_size):
            for i in reversed(range(rollout_steps[j])):
                ...
                kl_loss_vb[j] = kl_loss_vb[j] + klLoss(self.A3C_Experiences[j].posterior_vb[i],self.A3C_Experiences[j].prior_vb[i])
            
            loss_model_vb = loss_model_vb + (policy_loss_vb[j] + self.lam * value_loss_vb[j])/rollout_steps[j] + self.psi * kl_loss_vb[j]/rollout_steps[j]  
        ...
        for i in range(self.batch_size):
            ...
            kl_loss_avg += kl_loss_vb[i].data.cpu().numpy()/self.batch_size

        self.p_loss_avg   +=  p_loss_avg
        self.v_loss_avg   +=  v_loss_avg
        self.kl_loss_avg  +=  kl_loss_avg
        ...
        self.logger.warning("Iteration: {}; current kl_loss: {}; average kl_loss: {}".format(self.train_step, kl_loss_avg, self.kl_loss_avg/self.loss_counter))
