...
A3C_Experience = namedtuple('A3C_Experience', 'staten ,state0, action, reward, state1, terminal1, policy_vb, value0_vb, mu_vb, sigma_vb')
...
class A3CAgent(Agent):
        ...
        with torch.no_grad():
            ...
            while eps_count < self.max_episode and not test_done:
                test_p_vb, test_v_vb, test_lstm_hidden_vb = self.model(
                    preprocess_state(current_experience.state1, self.dtype, is_volatile=False), test_lstm_hidden_vb,
                    test_eps_variable)
    ...
    def _forward(self, state_vb):
        p_vb, v_vb, self.lstm_hidden_vb = self.model(state_vb, self.lstm_hidden_vb, self.eps_variable)
        if self.training:
